...
ZDCHitTree::ZDCHitTree(const std::string &name, const std::string &filename) 
  : SubsysReco(name)
  , nblocks(0)
  , hm(nullptr)
  , _filename(filename)
...
ZDCHitTree::~ZDCHitTree(){
  //  delete ntup;
  delete hm;
}

int ZDCHitTree::Init(PHCompositeNode *)
 {
   hm = new Fun4AllHistoManager(Name());
   outfile = new TFile(_filename.c_str(), "RECREATE");
   tree = new TTree("zdchit", "Collection of ZDC G4Hits");
 ...
 int ZDCHitTree::process_event(PHCompositeNode *topNode)
 {
 ...
 int ZDCHitTree::End(PHCompositeNode *topNode)
 {
   outfile->cd();
   tree->Write();
   outfile->Write();
 ...
 void ZDCHitTree::AddNode(const std::string &name, const int detid)
 {
