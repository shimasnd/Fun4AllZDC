...
// ZDCDetector::ConstructMe() method
// but the convention is as mentioned cm and deg
//____________________________________________________________________________..
//
#include "ZDCSubsystem.h"

#include "ZDCDetector.h"
#include "ZDCSteppingAction.h"
...
ZDCSubsystem::ZDCSubsystem(const std::string &name)
...
//_______________________________________________________________________
int ZDCSubsystem::InitRunSubsystem(PHCompositeNode *topNode)
{
  ...
  // create detector
  m_Detector = new ZDCDetector(this, topNode, GetParams(), Name());
  m_Detector->OverlapCheck(CheckOverlap());
  // create stepping action if detector is active
  if (GetParams()->get_int_param("active"))
  {
    m_SteppingAction = new ZDCSteppingAction(m_Detector, GetParams());
  }
  return 0;
}
//_______________________________________________________________________
int ZDCSubsystem::process_event(PHCompositeNode *topNode)
{
...
//_______________________________________________________________________
void ZDCSubsystem::Print(const string &what) const
{
  if (m_Detector)
  {
...
//_______________________________________________________________________
PHG4Detector *ZDCSubsystem::GetDetector(void) const
{
  return m_Detector;
}

//_______________________________________________________________________
void ZDCSubsystem::SetDefaultParameters()
{
  // sizes are in cm
  // angles are in rad
