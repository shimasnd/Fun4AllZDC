...
ZDCNtuple::ZDCNtuple(const std::string &name, const std::string &filename) 
  : SubsysReco(name)
  , nblocks(0)
  , hm(nullptr)
  , _filename(filename)
...
int ZDCNtuple::Init(PHCompositeNode *)
 {
 ...
 int ZDCNtuple::process_event(PHCompositeNode *topNode)
 {
 ...
 int ZDCNtuple::End(PHCompositeNode *topNode)
 {
   outfile->cd();
   ntup->Write();
   outfile->Write();
 ...
 void ZDCNtuple::AddNode(const std::string &name, const int detid)
 {
